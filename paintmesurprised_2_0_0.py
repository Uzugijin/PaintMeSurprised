...
class PMS_Properties(bpy.types.PropertyGroup):
    ...
    margin_size: bpy.props.EnumProperty(name="Bake Margin", description="Extends the baked result as a post process", items=[("0", "0", ""),("1", "1px", ""), ("2", "2px", ""), ("4", "4px", "")], default="2")
    ...
    auto_unwrap_enum: bpy.props.EnumProperty(name="Island gap", description="Gap size between uv islands", items=[("0.025", "Large", ""), ("0.005", "Medium", ""), ("0.0025", "Small", ""), ("0.0005", "Tiny", "")], default="0.025")
            ...
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                if pms_props.safe_to_run == 0:
                    ...
                    if any(obj in matching_objects for obj in bpy.context.selected_objects):
                        col.enabled = True
                        if pms_props.auto_unwrap == True:
                            col.operator("wm.retexel_operator", text="Retexel", icon="MOD_REMESH")
                        else:
