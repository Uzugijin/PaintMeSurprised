...
class PMS_Properties(bpy.types.PropertyGroup):
    ...
    base_scale: bpy.props.IntProperty(default=80)
...
class UVPixelCounter(bpy.types.Operator):
    ...
    def execute(self, context):
        pms_props = context.scene.pms_properties
        error_msg, count = self.auto_count_uv_pixels(pms_props.input_image)
        ...
        if error_msg:
            ...
            self.report({'INFO'}, f"Pixel Coverage: {formatted_count}px")
            print(f"SELECTED UV FACES COVER: {count} PIXELS")
                        
            # Store result for display in panel
            pms_props.uv_pixel_count = count
...
class UVLayoutManager(bpy.types.Operator):
    ...
    bl_description = "Unwraps selected in place"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        # Calculate scale ratio compared to UV space
        ...
        if pms_props.auto_unwrap == True and len(matching_objects) == 1:
            try:
                if any(p.select for p in mesh.polygons):
                    ...
                    base_scale = pms_props.base_scale / 1000
                    ...
                    bmesh.update_edit_mesh(mesh)
                    bpy.ops.mesh.select_all(action='SELECT')
                    bpy.ops.uv.pack_islands(rotate=False, scale=False, margin_method='ADD', margin=0.0005, shape_method='CONVEX')  
...
class UV_PT_PaintMeSurprised_picker(bpy.types.Panel):
    ...
    def poll(cls, context):
        try:
            return (context.object.mode == 'TEXTURE_PAINT' or context.object.mode == 'VERTEX_PAINT')
        except:
            return None
    ...
    def draw(self, context):
        ...
        if obj is not None:
            ...
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                row.operator("wm.reload_operator", text="", icon="PASTEDOWN")
                ...
                row = col.row(align=True)
                if pms_props.show_linked:
                    #row.operator("wm.pms_set_linked", text="Linked: "+str(len(matching_objects)), icon="DOWNARROW_HLT", emboss=False)
                    row.operator("wm.pms_set_linked", text="", icon="DOWNARROW_HLT", emboss=False)
                else:
                    #row.operator("wm.pms_set_linked", text="Linked: "+str(len(matching_objects)), icon="RIGHTARROW", emboss=False)
                ...
                row.operator("uv.auto_pixel_counter", text="", icon="BORDERMOVE")
                row = box3.row()
