...
class PMS_Properties(bpy.types.PropertyGroup):
    ...
    auto_unwrap_algo_enum: bpy.props.EnumProperty(name="Algorithm", description="Unwrap algorithm", items=[("SMART", "Smart UV Projection", ""), ("LIGHTMAP", "Lightmap Pack", ""), ("EQUAL", "Equal Faces", ""), ("VIEW", "Project from View", "")], default="SMART")
...
class UVLayoutManager(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, "UV layout stored")
        ...
        if error_msg:
            ...
            self.report({'INFO'}, "Unwrap completed and layout restored")
...
class RecOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if pms_props.auto_unwrap == True:
            ...
            elif pms_props.auto_unwrap_algo_enum == "VIEW":
                try:
                    if any(p.select for p in mesh.polygons):
                        bpy.ops.uv.project_from_view(orthographic=True, camera_bounds=False, correct_aspect=True, clip_to_bounds=True, scale_to_bounds=True)
                        bpy.ops.mesh.select_all(action='SELECT')
                        bpy.ops.uv.average_islands_scale()
                        bpy.ops.uv.pack_islands(rotate=True, scale=True, margin=float(pms_props.auto_unwrap_enum), shape_method='CONCAVE')
    ...
    def draw(self, context):
        ...
        if obj is not None:
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                box2 = box.box()
                row = box2.row()
                if pms_props.auto_unwrap == False:
                    row.enabled = False
                else:
                    row.enabled = True
