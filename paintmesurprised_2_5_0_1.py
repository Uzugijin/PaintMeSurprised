# Alpha color bake doesn't work on multiple objects unless all of those objects are instanced through geometry nodes.
...
class CleanPreviewOperator(bpy.types.Operator):
    ...
    bl_label = "Face Masking"
    bl_description = "Enable face masking and turn off overlays"
    bl_options = {'UNDO'}
    ...
    def execute(self, context):
        ...
        if pms_props.toggle_clean_preview == False:
            ...
            pms_props.previous_paint_state = context.object.data.use_paint_mask
            ...
            context.object.data.use_paint_mask = pms_props.previous_paint_state
...
class UVLayoutManager(bpy.types.Operator):
    bl_idname = "uv.layout_manager"
    bl_label = "Store & Restore UV Layout"
    bl_description = "Stores UV layout, unwraps, then restores position/size"
    bl_options = {'REGISTER', 'UNDO'}
    
    # Properties with defaults that will reset each time
    unwrap_method: bpy.props.EnumProperty(
        name="Unwrap Method",
        description="Method to use for unwrapping",
        items=[
            ('ANGLE_BASED', "Angle Based", "Use angle-based unwrapping"),
            ('PROJECT_FROM_VIEW', "Project from View", "Project UVs from current view")
        ],
        default='ANGLE_BASED'  # This will be the default every time
    )
    
    make_bbox_even: bpy.props.BoolProperty(
        name="Even Bounding Box",
        description="Make the bounding box square (equal width and height)",
        default=False  # This will be the default every time
    )
    ...
    def adjust_bbox_to_even(self, bbox):
        """Adjust bounding box to be square (equal width and height)"""
        min_u, min_v, max_u, max_v = bbox
        
        width = max_u - min_u
        height = max_v - min_v
        
        # Use the larger dimension for both sides
        ...
        # Create new square bbox centered on original center
    ...
    def restore_uv_bbox(self, original_bbox):
        """Scales and moves selected UV faces to match the original bounding box"""
        obj = bpy.context.active_object
        if not obj or obj.type != 'MESH':
        ...
        # Get current bbox of selected faces
        all_uvs = [loop[uv_layer].uv for face in selected_faces for loop in face.loops]
        u_coords = [uv.x for uv in all_uvs]
        v_coords = [uv.y for uv in all_uvs]
        
        current_bbox = (min(u_coords), min(v_coords), max(u_coords), max(v_coords))
        curr_min_u, curr_min_v, curr_max_u, curr_max_v = current_bbox
        orig_min_u, orig_min_v, orig_max_u, orig_max_v = original_bbox
        
        # Calculate scale factors
        current_width = curr_max_u - curr_min_u
        current_height = curr_max_v - curr_min_v
        target_width = orig_max_u - orig_min_u
        target_height = orig_max_v - orig_min_v
        
        scale_u = target_width / current_width if current_width > 0 else 1
        scale_v = target_height / current_height if current_height > 0 else 1
        
        # Transform UVs to match original bbox
        for face in selected_faces:
            for loop in face.loops:
                uv = loop[uv_layer]
                # Scale and reposition
                uv.uv.x = ((uv.uv.x - curr_min_u) * scale_u) + orig_min_u
                uv.uv.y = ((uv.uv.y - curr_min_v) * scale_v) + orig_min_v
        
        bmesh.update_edit_mesh(mesh)
        return None
    
    def perform_unwrap(self):
        """Perform the selected unwrap method"""
        try:
            if self.unwrap_method == 'ANGLE_BASED':
                bpy.ops.uv.unwrap(method='ANGLE_BASED', margin=0.001)
            elif self.unwrap_method == 'PROJECT_FROM_VIEW':
                bpy.ops.uv.project_from_view()
        except Exception as e:
            return f"Unwrap failed: {str(e)}"
        return None
    
    def invoke(self, context, event):
        # Reset properties to defaults when operator is invoked
        self.unwrap_method = 'ANGLE_BASED'
        self.make_bbox_even = False
        return self.execute(context)
    
    def execute(self, context):
        pms_props = context.scene.pms_properties    
        # Full workflow: Store → Unwrap → Restore
        error_msg, bbox = self.store_uv_bbox()
        if error_msg:
            self.report({'ERROR'}, error_msg)
            return {'CANCELLED'}
        
        # Apply even bounding box if enabled
        if self.make_bbox_even:
            bbox = self.adjust_bbox_to_even(bbox)
        
        pms_props.stored_uv_bbox = bbox
        
        # Perform unwrap
        error_msg = self.perform_unwrap()
        if error_msg:
            self.report({'ERROR'}, error_msg)
            return {'CANCELLED'}
        
        # Restore layout
        error_msg = self.restore_uv_bbox(bbox)
        if error_msg:
            self.report({'ERROR'}, error_msg)
        else:
            pass
    ...
    def draw(self, context):
        ...
        layout.prop(self, "make_bbox_even")
        ...
        if pms_props.auto_unwrap == True and len(matching_objects) == 1:
            try:
                if any(p.select for p in mesh.polygons):
                    ...
                    base_scale = 0.01
                    base_margin = 0.0005
                    if pms_props.auto_unwrap_enum == "0.025":
                        base_margin = base_margin*1
                        base_scale = base_scale*10
                    elif pms_props.auto_unwrap_enum == "0.005":
                        base_margin = base_margin*2
                        base_scale = base_scale*5
                    elif pms_props.auto_unwrap_enum == "0.0025":
                        base_margin = base_margin*4
                        base_scale = base_scale*2.5
                    elif pms_props.auto_unwrap_enum == "0.0005":
                        base_margin = base_margin*8
                        base_scale = base_scale*1

                    # Combine area and triangle count for scaling
                    area_percentage = selected_area / 100 
                    ...
                    bmesh.update_edit_mesh(mesh)
                    bpy.ops.mesh.select_all(action='SELECT')
                    bpy.ops.uv.pack_islands(rotate=False, scale=False, margin_method='ADD', margin=base_margin, shape_method='CONVEX')  
        ...
        if obj is not None:
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                if pms_props.input_image == "" or obj.mode != 'EDIT':
                    row.enabled = False
                row.operator("uv.layout_manager", text="Unwrap", icon="GROUP_UVS")
                row = box3.row()
