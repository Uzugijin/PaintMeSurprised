...
def copyTextureAndUVMAP(arg1, arg2):
    ...
    if arg2:
        ...
        uvmap_copy.name = f"{arg2}{TEMP_SUFFIX}"
        obj.data.uv_layers.active = uvmap_copy

    return copy_texture, uvmap_copy
...
def Record(og_tex_name, current_active_uv):
    ...
    _, uvmap_copy = copyTextureAndUVMAP("", current_active_uv)
    copy_texture, _ = copyTextureAndUVMAP(og_tex_name, "")
...
def Stop(og_tex_name, current_active_uv):
    ...
    if image_node_of_copy and image_node_of_copy.image:
        # Get the image data from the selected node
        source_image = image_node_of_copy.image
