...
def auto_count_uv_pixels(input_image):
...
class UVPixelCounter(bpy.types.Operator):
    ...
    def execute(self, context):
        pms_props = context.scene.pms_properties
        error_msg, count = auto_count_uv_pixels(pms_props.input_image)
        
        if error_msg:
            self.report({'ERROR'}, error_msg)
        else:
            # Display in UI and console
            #self.report({'INFO'}, f"Pixel Coverage: {count}px")
            #print(f"SELECTED UV FACES COVER: {count} PIXELS")
            
            # Store result for display in panel
            context.scene.uv_pixel_count = count
        ...
        if obj is not None:
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                if pms_props.input_image == "" or obj.mode != 'EDIT':
                    row.enabled = False
                row.operator("uv.auto_pixel_counter", text=f"UV Area: {getattr(context.scene, 'uv_pixel_count', 0)} px", icon="EYEDROPPER")
                row = box3.row()
