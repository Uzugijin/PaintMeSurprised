...
class PMS_Properties(bpy.types.PropertyGroup):
    ...
    sensor_enum: bpy.props.EnumProperty(name="Threshold", description="Determines which faces are considered narrow", items=[("0.0005", "3", ""), ("0.0002", "2", ""), ("0.0001", "1", ""), ("0", "0", "")], default="0.0001")
...
class UnfudgeOperator(bpy.types.Operator):
    ...
    # Get the active object and its UV data
        ...
        threshold = float(pms_props.sensor_enum)
        narrow_faces = 0
        ...
        for face in obj.data.polygons:
            ...
            for i in range(len(face_uv_coords)):
                v1 = face_uv_coords[i]
                v2 = face_uv_coords[(i+1) % len(face_uv_coords)]
        ...
        if narrow_faces > 0:
            ...
            self.report({'INFO'}, "Narrow faces found: {}".format(narrow_faces))
        else:
            bpy.ops.object.mode_set(mode=mode_before)
            self.report({'WARNING'}, "No narrow faces found!")
        return {'FINISHED'}
        ...
        if obj is not None:
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                if pms_props.auto_unwrap == True:
                    ...
                    row.operator("wm.unfudge", text="Unwrap Narrow Faces")
                    row = box2.row()
                if pms_props.auto_unwrap == True:
                        row.enabled = False
                else:
                ...
                row.prop(pms_props, "sensor_enum", text="")
                row = box2.row()
