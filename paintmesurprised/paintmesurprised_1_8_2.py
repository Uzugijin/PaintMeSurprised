...
class PMS_Properties(bpy.types.PropertyGroup):
    ...
    bake_type: bpy.props.EnumProperty(name="Bake Type", description="Bake type", items=[("EMIT", "Emission", ""),("COMBINED", "Combined", ""),("DIFFUSE", "Diffuse", ""),("NORMAL", "Normal", "")], default="EMIT")
...
def Stop(input_image, input_uv, temp_suffix, mix_method, checkpoint_suffix, uvmap_copy_name, margin_size, is_bakefullchain, bake_type):

    def mixing_procedure():
        ...
        making_image_axtive(input_image)
...
class BakeEmitChain(bpy.types.Operator):
    ...
    bl_description = "Bakes RGBA or emission shaders into an image. Works best if all materials are emissive."
    bl_options = {'INTERNAL'}
    ...
    def execute(self, context):
        ...
        Stop(pms_props.input_image, pms_props.input_uv, "_bakedfullchain", pms_props.image_mode, pms_props.checkpoint_suffix, pms_props.uvmap_copy_ref, pms_props.margin_size, True, pms_props.bake_type)
            ...
            if pms_props.safe_to_run == 2:
                try:
                    try:
                        ...
                        pass
                    Stop(pms_props.input_image, pms_props.input_uv, pms_props.temp_suffix, pms_props.image_mode, pms_props.checkpoint_suffix, pms_props.uvmap_copy_ref, pms_props.margin_size, False, "EMIT")
                except Exception as e:
...
class UV_PT_PaintMeSurprised(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        matching_objects = get_matching_objects(pms_props.input_uv, str(pms_props.input_image))
        selected_objectos = bpy.context.selected_objects
        image_users = get_image_users(str(pms_props.input_image))
        ...
        if obj is not None:
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                if pms_props.image_mode == 'transfer':
                    ...
                    row.label(text="Mixing (A+B)", icon="RENDERLAYERS")
                row = layout.row()
                row.prop(pms_props, "image_mode", text="Mode")
                row = layout.row()
                ...
                if shortcut == True:
                    ...
                    row.enabled = True                 
                row.operator("wm.bake_emit_chain", text="Bake:", icon="OUTPUT")
                row.prop(pms_props, "bake_type", text="")
                row = layout.row()
                row.label(text=":", icon="NORMALS_FACE")
                if pms_props.paint_both == True:
                    row.operator("wm.paint_both_sides", text="", icon="RADIOBUT_ON")
                else:
                    row.operator("wm.paint_both_sides", text="", icon="RADIOBUT_OFF")
                #row = layout.row()
                row.label(text=":", icon="OVERLAY")
                if pms_props.toggle_clean_preview == True:
                    row.operator("wm.clean_preview", text="", icon="RADIOBUT_ON")
                else:
                    row.operator("wm.clean_preview", text="", icon="RADIOBUT_OFF")
            ...
            row = layout.row()
            layout.label(text="No object is selected!", icon="ERROR")
