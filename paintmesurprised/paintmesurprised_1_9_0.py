...
class PMS_Properties(bpy.types.PropertyGroup):
    ...
    bake_type: bpy.props.EnumProperty(name="Bake Type", description="Bake type", items=[("EMIT", "Emission", ""),("COMBINED", "Combined", ""),("NORMAL", "Normal", "")], default="EMIT")
    ...
    def execute(self, context):
        ...
        for mat in bpy.data.materials:
            if mat.use_nodes:
                ...
                if active_node and active_node.type == 'TEX_IMAGE' and active_node.image and active_node.image.name == pms_props.input_image:
                    self.report({'ERROR'}, "Input image is PROTECTED! Select another image node.")
                    return {'CANCELLED'}
