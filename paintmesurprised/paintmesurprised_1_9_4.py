...
class BakeEmitChain(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.object.mode_set(mode='OBJECT')
        copyTexture(pms_props.input_image, pms_props.checkpoint_suffix)
...
class UV_PT_PaintMeSurprised(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        matching_objects = get_image_users(str(pms_props.input_image))
        ...
        if obj is not None:
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                col.label(text="UV: "+pms_props.input_uv)
                row = box.row(align=True)
