...
def Stop(input_image, input_uv, temp_suffix, mix_method, checkpoint_suffix, uvmap_copy_name):
        ...
        if material.use_nodes:
            ...
            for node in node_tree.nodes:
                if node.type == 'TEX_IMAGE' and node.image and node.image.name == input_image:
                    ...
                    print("PRINTING THE")
                    print(f"uvnode_of_og_tex_pms: {uvnode_of_og_tex_pms.name}")
                    print(f"uvnode_of_copy_pms: {uvnode_of_copy_pms.name}")
    ...
    connectNodesForBaking(input_image, temp_suffix)
...
def cleanup_temp_nodes(temp_suffix):
    ...
    if image_node_of_og_tex_pms is not None:
        try:
...
class UV_request(bpy.types.Operator):
    ...
    bl_label = "Match UV"
    bl_description = "Calculate the difference in UV size from the last checkpoint."
    bl_options = {'INTERNAL'}
...
class CleanPreviewOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        pms_props = context.scene.pms_properties
        print(pms_props.toggle_clean_preview)
...
class StopOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if pms_props.lock  == False:
            ...
            if pms_props.safe_to_run == 2:
                try:
                    try:
                        ...
                        pass
                    Stop(pms_props.input_image, pms_props.input_uv, pms_props.temp_suffix, pms_props.image_mode, pms_props.checkpoint_suffix, pms_props.uvmap_copy_ref)
                except Exception as e:
            ...
            cleanup_temp_nodes(pms_props.temp_suffix)
            cleanup_temp_data(pms_props.temp_suffix, pms_props.input_uv, pms_props.input_image)
            print(pms_props.mode_before_record + "PRINT ME")
            bpy.ops.object.mode_set(mode=pms_props.mode_before_record)
            for uv in bpy.context.object.data.uv_layers:
                if uv.name == pms_props.input_uv:
                    bpy.context.object.data.uv_layers.active = uv 
        ...
        if obj is not None:
            if all(obj.type == 'MESH' for obj in bpy.context.selected_objects) and obj.type == 'MESH':
                ...
                if pms_props.paint_both == True:
                    row.operator("wm.paint_both_sides", text="Paint Both Sides", icon="RADIOBUT_ON")
                else:
                    row.operator("wm.paint_both_sides", text="Paint Both Sides", icon="RADIOBUT_OFF")
                row = layout.row()
                if pms_props.toggle_clean_preview == True:
                    row.operator("wm.resize_image", text="Clean Preview", icon="RADIOBUT_ON")
                else:
                    row.operator("wm.resize_image", text="Clean Preview", icon="RADIOBUT_OFF")
